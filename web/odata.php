 ...
 * (nightly/on-demand via odata_get_all / project_fetch_rows) naar Mímir i.p.v. BC.
 *
 * Met $mimirApi gezet zijn $auth_list / $environment / $baseUrl / $auth ongebruikt voor BC;
 * Mímir beheert environments — Juna-Moneta heeft alleen de API-key (+ optioneel $mimirBase) nodig.
 *
 ...
 *
 * max_age-beleid: nightly.php gebruikt MONETA_NIGHTLY_MAX_AGE (14400);
 * UI/on-demand houdt MONETA_GL_ODATA_TTL (604800). Geen hourly.php.
 */

/** Mímir max_age for nightly.php snapshot builds (4h). */
const MONETA_NIGHTLY_MAX_AGE = 14400;
...
function odata_mimir_request(string $method, string $path, ?array $jsonBody = null): array
{
    ...
    $opts = [
        ...
        CURLOPT_USERAGENT => 'JunaMoneta-MimirClient/1.0',
    ];
    if ($jsonBody !== null) {
...
function odata_get_all(string $url, array $auth, $ttlSeconds = 300): array
{
    ...
    if (odata_mimir_api_key() !== '') {
        // Mímir beheert de BC-cache (max_age); Juna-Moneta-filecache / live-paginering worden overgeslagen.
